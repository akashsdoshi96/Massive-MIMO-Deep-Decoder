...
H_org1 = zeros(Nsub,1,Nr,n_chn_realizations);
H_fts = zeros(Nsub,Nsub,Nr,n_chn_realizations);
...
%     for ant = 1:64
%         tap_1_cg = randn(1,1) + 1j*randn(1,1);
%         dft_64 = fft(tap_1_cg,64);
%         H_fts(:,:,ant,ii) = repmat(reshape(dft_64,[64,1]),1,64);
%     end
%     for j = 1:Nsub
%         H_org1(j,1,:,ii) = reshape((R^(1/2))*reshape(H_fts(j,1,:,ii),[Nr,1]),[1,Nr]);
%     end
%     H_org(:,:,:,ii) = repmat(H_org1(:,1,:,ii),1,64,1,1);
...
for ii = 1:n_chn_realizations
    ...
    H_fts(:,:,:,ii) = repmat(H_relevant, 1, Nsub/8, 1);
    for k = 1:Nsub
       for j = 1:Nsub 
           H_org(j,k,:,ii) = reshape((R^(1/2))*reshape(H_fts(j,k,:,ii),[Nr,1]),[1,Nr]);
       end
    end
    H_2D = reshape(H_org(1:Nsub,1,:,ii),[Nsub*Nr,1]);
    Rhh = Rhh + (1/n_chn_realizations)*(H_2D*H_2D');
...
save('H_64x64x64_exp.mat','noiseless','noisy_0dB','noisy_5dB','noisy_10dB','noisy_15dB','noisy_20dB');
