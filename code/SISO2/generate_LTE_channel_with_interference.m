...
for iter = 1:n_chn_realizations 
    ...
    for k = 1:length(SNR_vec)
        ...
        Rhh_interference = interference(:,1,iter)*interference(:,1,iter)';
        H_LMMSE = Rhh*((Rhh+(power(10,-snr/10))*(diag(abs(H_org(:,1,iter))).*eye(Nsub)))\H_ls);
        H_LMMSE_true = Rhh_true*((Rhh_true+Rhh_interference+(power(10,-snr/10))*(diag(abs(H_org(:,1,iter))).*eye(Nsub)))\H_ls);
